...
`Optim.OptimizationOptions`.
```jlcon
julia> Optim.minimizer(optimize(f, initial_x, BFGS(), Optim.OptimizationOptions(autodiff = true)))
2-element Array{Float64,1}:
 1.0
 1.0

julia> Optim.minimizer(optimize(f, initial_x, Newton(), Optim.OptimizationOptions(autodiff = true)))
2-element Array{Float64,1}:
 1.0
 1.0
```
