...
the gradient.
```jl
optimize(f, [0.0, 0.0], LBFGS(), Optim.OptimizationOptions(autodiff = true))
```
...
```julia
results = Optim.optimize(DifferentiableFunction(f), initial_x, l, u, Fminbox(); optimizer = GradientDescent, optimizer_o = Optim.OptimizationOptions(iterations = 2))
```
