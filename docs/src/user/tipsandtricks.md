...
julia> @time optimize(f, initial_x, GradientDescent(),
                      Optim.OptimizationOptions(show_trace=true, iterations = 5))
...
do this is to set the `iterations` keyword in `Optim.OptimizationOptions` to some number.
...
the optimization procedure by setting the `time_limit` keyword in the `Optim.OptimizationOptions`
constructor.
```julia
using Optim
...
optimize(slow, zeros(2), NelderMead(), Optim.OptimizationOptions(time_limit = 3.0))
```
...
optimize(very_slow, zeros(2), NelderMead(), Optim.OptimizationOptions(callback = advanced_time_control))
```
...
```jlcon
julia> optimize(very_slow, zeros(2), NelderMead(), Optim.OptimizationOptions(callback = advanced_time_control))
 * Iteration:       0
