let
    ...
    Optim.optimize(DifferentiableFunction(f_1, g!_1), [0.0], Newton(), Optim.OptimizationOptions(autodiff = true))
    ...
    for (name, prob) in Optim.UnconstrainedProblems.examples
    	if prob.istwicedifferentiable
    		...
    		res = Optim.optimize(ddf, prob.initial_x, Newton(), Optim.OptimizationOptions(autodiff = true))
    		@assert norm(Optim.minimizer(res) - prob.solutions) < 1e-2
    		res = Optim.optimize(ddf.f, prob.initial_x, Newton(), Optim.OptimizationOptions(autodiff = true))
    		@assert norm(Optim.minimizer(res) - prob.solutions) < 1e-2
            res = Optim.optimize(ddf.f, ddf.g!, prob.initial_x, Newton(), Optim.OptimizationOptions(autodiff = true))
    		@assert norm(Optim.minimizer(res) - prob.solutions) < 1e-2
    	end
    end
end
