...
let
   ...
   Optim.optimize(f, initial_x, SimulatedAnnealing())
    ...
    res = optimize(f, g!, h!,
    	           initial_x,
    	           NelderMead(),
    	           OptimizationOptions(f_tol = 1e-12,
    	           iterations = 10,
    	           store_trace = true,
    	           show_trace = false))
    ...
    res = optimize(f, g!, h!,
    	           initial_x,
    	           SimulatedAnnealing(),
    	           OptimizationOptions(iterations = 10,
    	           store_trace = true,
    	           show_trace = false))
   ...
   res_extended = Optim.optimize(f, g!, initial_x, BFGS(), OptimizationOptions(store_trace = true, extended_trace = true))
   ...
   res_extended_nm = Optim.optimize(f, g!, initial_x, NelderMead(), OptimizationOptions(store_trace = true, extended_trace = true))
